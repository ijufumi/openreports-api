...
object ReportReportParameterConverter {
  ...
  object conversions {
    ...
    implicit def toReportReportParameterEntity(
        model: ReportReportParameterModel,
    ): ReportReportParameterEntity = toEntity(model)
    implicit def toReportReportParameterEntities(
        model: Seq[ReportReportParameterModel],
    ): Seq[ReportReportParameterEntity] = model.map(toEntity)
    implicit def fromReportReportParameterEntity(
        entity: ReportReportParameterEntity,
    ): ReportReportParameterModel = toDomain(entity)
    implicit def fromReportReportParameterEntities(
        entity: Seq[ReportReportParameterEntity],
    ): Seq[ReportReportParameterModel] = entity.map(toDomain)
  }
}
